/** Shared InnerTube player logic for Netlify Edge (Deno) — keep in sync with youtube-clients.ts */
...
async function postPlayer(videoId: string, client: (typeof CLIENTS)[number]): Promise<RawPlayer | null> {
...
export interface EdgeMediaInfo {
  id: string;
  title: string;
  thumbnail?: string;
  duration?: number;
...
export function mapPlayerToMediaInfo(
  data: RawPlayer,
  videoId: string,
  webpageUrl: string
): EdgeMediaInfo | null {
...
export async function fetchYoutubeOnEdge(url: string): Promise<EdgeMediaInfo | null> {
